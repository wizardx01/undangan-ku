<template>
  <div class="list-dashboard">
    ...
    <div class="create-section">
      ...
      <div class="event-type-grid">
        <div class="event-card" @click="createNewWedding('wedding')">
          <div class="event-icon">💍</div><h3>Wedding</h3><p>Pernikahan</p>
        </div>
        <div class="event-card" @click="createNewWedding('sunatan')">
          <div class="event-icon">✂️</div><h3>Sunatan</h3><p>Khitanan</p>
        </div>
        <div class="event-card" @click="createNewWedding('aqiqah')">
          <div class="event-icon">👶</div><h3>Aqiqah</h3><p>Syukuran Kelahiran</p>
        </div>
        <div class="event-card" @click="createNewWedding('syukuran')">
          <div class="event-icon">🏠</div><h3>Syukuran</h3><p>Rumah Baru, Wisuda, dll</p>
        </div>
      </div>
          ...
          <div class="wedding-info">
            ...
            <div class="wedding-details">
              ...
              <div class="wedding-stats">
                ...
                <span :class="wedding.is_active ? 'status-active' : 'status-inactive'">{{ wedding.is_active ? '🟢 Aktif' : '🔴 Nonaktif' }}</span>
              </div>
            </div>
          </div>
    ...
    <div v-if="!loading && weddings.length === 0" class="empty"><p>📭 Belum ada undangan</p><p>Pilih jenis acara di atas untuk memulai!</p></div>
...
const showLimitWarning = computed(() => user.value?.role !== 'admin' && !canCreate.value)
...
const loadWeddings = async () => {
  ...
  try {
    ...
    if (user.value?.role !== 'admin') query = query.eq('user_id', user.value.id)
    const { data, error } = await query
    if (error) throw error
    for (const w of data || []) {
      const { count: gc } = await supabase.from('guests').select('*', { count: 'exact', head: true }).eq('wedding_id', w.id)
      const { count: gic } = await supabase.from('gifts').select('*', { count: 'exact', head: true }).eq('wedding_id', w.id).eq('status', true)
      w.guest_count = gc || 0; w.gift_count = gic || 0
    }
    weddings.value = data || []
...
const createNewWedding = (eventType) => {
  if (!canCreate.value) { alert('❌ Limit undangan tercapai!'); return }
  localStorage.removeItem('currentWeddingId'); localStorage.removeItem('currentSlug')
  sessionStorage.setItem('selectedEventType', eventType)
  router.push('/dashboard/edit')
}

const editWedding = (w) => { localStorage.setItem('currentWeddingId', w.id); localStorage.setItem('currentSlug', w.slug); router.push('/dashboard/edit') }
const copyLink = (slug) => { navigator.clipboard.writeText(`${window.location.origin}/wedding/${slug}`); showSuccess('Link dicopy!') }
const previewWedding = (slug) => window.open(`/wedding/${slug}`, '_blank')

const deleteWedding = async (w) => {
  if (!confirm(`Hapus undangan ini?`)) return
  try {
    await supabase.from('gifts').delete().eq('wedding_id', w.id)
    await supabase.from('messages').delete().eq('wedding_id', w.id)
    await supabase.from('guests').delete().eq('wedding_id', w.id)
    await supabase.from('weddings').delete().eq('id', w.id)
    weddings.value = weddings.value.filter(x => x.id !== w.id)
    showSuccess('Dihapus!')
  } catch (err) { showError(err, 'Gagal') }
}
...
onMounted(async () => {
  // 1. Cek session custom
  const session = JSON.parse(localStorage.getItem('session') || 'null')
  
  // 2. Cek session Supabase Auth (Google)
...
.event-card:hover { border-color: #9b87f5; background: #f5f0ff; transform: translateY(-5px); }
.event-icon { font-size: 48px; margin-bottom: 12px; }
.event-card h3 { font-size: 16px; color: #2c3e50; }
...
.wedding-card { background: white; padding: 20px; border-radius: 16px; box-shadow: 0 5px 20px rgba(0,0,0,0.05); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px; }
