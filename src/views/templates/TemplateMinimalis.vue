<template>
  <div class="template-minimalis">
    <div v-if="!isOpen" class="cover-minimalis">
      <div class="cover-card"><div class="cover-icon">{{ getCoverIcon() }}</div><h1 class="cover-title" v-html="getTitle()"></h1><p class="cover-date">{{ formatDate(props.wedding.akad_date) }}</p><p class="cover-guest">Kepada Yth.<br><strong>{{ displayGuestName }}</strong></p><button class="btn-open" @click="openEnvelope">💌 Buka Undangan</button><button class="music-toggle" @click="toggleMusic" :class="{ playing: isPlaying }">{{ isPlaying ? '🔊' : '🔇' }}</button></div>
    </div>
    
    <div v-else class="content-minimalis"><div class="container">
      ...
      <div class="countdown"><h3>⏳ Menuju Hari Bahagia</h3><div class="countdown-grid"><div class="countdown-item"><span class="value">{{ countdown.days }}</span><span class="label">Hari</span></div><div class="countdown-item"><span class="value">{{ countdown.hours }}</span><span class="label">Jam</span></div><div class="countdown-item"><span class="value">{{ countdown.minutes }}</span><span class="label">Menit</span></div></div></div>
      <div class="guest-section"><p>Yang terhormat</p><p class="guest-name-display">{{ displayGuestName }}</p></div>
      
      <div class="event"><h3>{{ getEventIcon() }} {{ getEventLabel() }}</h3><p><span>📆</span> {{ formatDate(props.wedding.akad_date) }}</p><p><span>🕐</span> {{ formatTime(props.wedding.akad_time) }}</p><p><span>📍</span> {{ props.wedding.akad_location }}</p><button class="btn-map" @click="openMaps(props.wedding.akad_location)">🗺️ Buka Google Maps</button><div class="calendar-section" v-if="props.wedding.akad_date"><add-to-calendar-button :name="getTitleText()" :startDate="formatCalendarDate(props.wedding.akad_date)" :startTime="props.wedding.akad_time?.substring(0,5)" :location="props.wedding.akad_location" options="'Google'" timeZone="Asia/Jakarta" hideTextLabelButton></add-to-calendar-button></div></div>
      ...
      <div class="rsvp"><h3>Konfirmasi Kehadiran</h3><div class="rsvp-buttons"><button @click="rsvpStatus = true" :class="{ active: rsvpStatus === true }">✅ Hadir</button><button @click="rsvpStatus = false" :class="{ active: rsvpStatus === false }">❌ Tidak</button></div><button @click="submitRsvp" :disabled="submitting" class="btn-submit">Kirim Konfirmasi</button></div>
      <div class="messages"><h3>💬 Ucapan & Doa</h3><div class="msg-list"><div v-for="msg in messages" :key="msg.id" class="msg-item"><strong>{{ msg.nama_pengirim }}</strong><p>{{ msg.pesan }}</p><small>{{ formatTime(msg.created_at) }}</small></div></div><div class="msg-form"><input v-model="newMessage.nama" placeholder="Nama" /><textarea v-model="newMessage.pesan" placeholder="Ucapan..." rows="2"></textarea><button @click="sendMessage" :disabled="sendingMessage">Kirim Doa</button></div></div>
...
.music-toggle { position: absolute; bottom: 20px; right: 20px; background: white; border: none; font-size: 24px; padding: 12px; border-radius: 50%; cursor: pointer; }
...
.love-story-section h3 { text-align: center; margin-bottom: 20px; color: #2c3e50; }
.timeline { position: relative; padding-left: 30px; border-left: 2px solid #2c3e50; }
.timeline-item { display: flex; gap: 15px; margin-bottom: 25px; position: relative; }
.timeline-icon { width: 40px; height: 40px; background: #2c3e50; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 18px; position: absolute; left: -51px; }
