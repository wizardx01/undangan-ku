<template>
  <div class="wedding-view template-elegan">
    ...
    <div class="floating-elements">
      <span v-for="i in 15" :key="i" class="floating-leaf" :style="{ '--delay': i * 0.4 + 's', '--pos': i * 7 + '%' }">🌸</span>
    ...
    <div v-if="!isOpen" class="cover-page" :style="coverStyle">
      <div class="cover-content" :class="{ 'tearing': isTearing }">
        <div class="cover-frame">
          ...
          <h2 class="cover-subtitle">The Wedding Of</h2>
          ...
          <button class="btn-open" @click="openEnvelope">
            <span>💌 Buka Undangan</span>
          </button>
        </div>
        
        <button class="music-toggle cover-music" @click="toggleMusic" :class="{ playing: isPlaying }">
          {{ isPlaying ? '🔊' : '🔇' }}
        </button>
      </div>
    </div>
    ...
    <div v-else class="main-content">
      <div class="content-card">
        ...
        <div class="top-actions">
          <button class="icon-btn" @click="toggleMusic">
            <span>{{ isPlaying ? '🔊' : '🔈' }}</span>
          </button>
        </div>
        ...
        <div class="hero-section" data-aos="fade-up">
          <div class="hero-image" v-if="props.theme.gallery && props.theme.gallery.length">
            <img :src="props.theme.gallery[0]" alt="Couple" />
            <div class="hero-overlay"></div>
          </div>
          <div class="hero-content">
            <div class="bismillah">﷽</div>
            <p class="hero-text">Dengan memohon rahmat dan ridho Allah SWT, kami mengundang Bapak/Ibu/Saudara/i untuk menghadiri pernikahan:</p>
          </div>
        </div>
        ...
        <div class="section-names" data-aos="fade-up" data-aos-delay="100">
          <div class="names-container">
            <h1 class="wedding-title">{{ props.wedding.nama_pria }}</h1>
            <div class="and-symbol">&</div>
            <h1 class="wedding-title">{{ props.wedding.nama_wanita }}</h1>
          </div>
          <p class="parents-name">Putra dari {{ props.wedding.orangtua_pria || '...' }}<br>&<br>Putri dari {{ props.wedding.orangtua_wanita || '...' }}</p>
        </div>
        ...
        <div class="section-countdown" data-aos="fade-up" data-aos-delay="300">
          ...
          <div class="countdown-container" v-if="countdownTarget">
            ...
            <p class="countdown-target-label">Menuju {{ countdownTarget.label }}</p>
          </div>
        </div>
        ...
        <div class="section-event" data-aos="fade-up" data-aos-delay="500">
          ...
          <div class="event-card resepsi-card" v-if="props.wedding.resepsi_date && props.wedding.resepsi_time">
            <div class="event-header">
              <div class="event-icon">🎉</div>
        ...
        <div class="section-gallery" data-aos="fade-up" data-aos-delay="600" v-if="props.theme.gallery && props.theme.gallery.length">
          ...
          >
            <swiper-slide v-for="(img, i) in props.theme.gallery" :key="i">
              <div class="gallery-slide" @click="openLightbox(i)">
                <img :src="img" />
              </div>
            </swiper-slide>
          </swiper>
        ...
        <div class="section-wishes" data-aos="fade-up" data-aos-delay="700">
          <div class="wishes-box">
            ...
            <p class="wishes-text">
              "Semoga Allah mengumpulkan kami berdua dalam kebaikan,<br>
              menjadikan rumah tangga kami sakinah, mawaddah, warahmah,<br>
              dan mengaruniakan keturunan yang shalih dan shalihah."
            </p>
            <p class="wishes-signature">— {{ props.wedding.nama_pria }} & {{ props.wedding.nama_wanita }}</p>
          </div>
        </div>
        ...
        <div class="section-gifts" data-aos="fade-up" data-aos-delay="800" v-if="gifts.length > 0">
          ...
          <p class="gift-message">Berikut beberapa kado yang diinginkan oleh kedua mempelai:</p>
          
          <div class="gift-grid">
            <div v-for="gift in gifts" :key="gift.id" class="gift-card" :class="{ dibeli: gift.status }">
              <div class="gift-card-image" v-if="gift.image">
                <img :src="gift.image" />
              </div>
              <div class="gift-card-content">
                ...
                <p class="gift-hint" v-if="!gift.status">*Setelah beli, input resi di bawah</p>
              </div>
              
              <div v-if="props.guestSlug && !gift.status" class="resi-form">
                <input v-model="gift.resiInput" placeholder="Nomor Resi" />
                <button @click="claimGift(gift)" :disabled="gift.claiming">{{ gift.claiming ? '...' : 'Klaim' }}</button>
              </div>
              
              <div v-if="gift.buyer_name" class="gift-buyer-info">
                Dibeli oleh: {{ gift.buyer_name }}
        ...
        <div class="section-gift" data-aos="fade-up" data-aos-delay="900" v-if="props.wedding.rekening">
          ...
          <p class="gift-message">Doa restu Anda adalah hadiah terindah. Namun jika ingin memberi tanda kasih:</p>
          
          <div class="rekening-list">
            <div class="rekening-item">
              <pre>{{ props.wedding.rekening }}</pre>
              <button @click="copyRekening" :class="{ shaking: isShaking }">
                <span>📋</span> Salin
              </button>
            </div>
          </div>
        ...
        <div class="section-rsvp" data-aos="fade-up" data-aos-delay="1000">
          ...
          <div v-if="rsvpStatus === true" class="rsvp-guest-count">
            <label>Jumlah Orang</label>
            <input type="number" v-model="jumlahOrang" min="1" max="10" />
          </div>
          ...
          <div class="floating-hearts" v-if="showHearts">
            <span v-for="i in 15" :key="i" class="heart" :style="{ '--i': i }">❤️</span>
          </div>
        </div>
        ...
        <div class="section-messages" data-aos="fade-up" data-aos-delay="1100">
          ...
          <div class="message-list">
            ...
            <div v-if="messages.length === 0" class="message-empty">💭 Belum ada ucapan. Jadilah yang pertama!</div>
          </div>
          <div class="message-form">
            <input v-model="newMessage.nama" placeholder="Nama Anda" />
            <textarea v-model="newMessage.pesan" placeholder="Tulis doa atau ucapan..." rows="3"></textarea>
            <button @click="sendMessage" :disabled="sendingMessage"><span>📨</span> Kirim Doa</button>
          </div>
        </div>
        ...
        <div class="section-closing" data-aos="fade-up" data-aos-delay="1200">
          ...
          <p class="closing-text">Merupakan suatu kehormatan dan kebahagiaan bagi kami<br>atas kehadiran Bapak/Ibu/Saudara/i untuk memberikan doa restu.</p>
          <p class="closing-signature">Wassalamu'alaikum Wr. Wb.</p>
          <div class="virtual-signature">{{ props.wedding.nama_pria }} & {{ props.wedding.nama_wanita }}</div>
...
const bgStyle = computed(() => ({
  backgroundImage: props.theme.background_type === 'image' ? `url(${props.theme.background_value})` : 'none',
  backgroundColor: props.theme.background_type === 'solid' ? props.theme.background_value : 'transparent'
}))

const coverStyle = computed(() => ({
  '--primary': props.theme.primary_color || '#9b87f5',
  fontFamily: props.theme.font_family || 'Poppins, sans-serif'
}))
...
const countdownTarget = computed(() => {
  ...
  if (props.wedding.akad_date && props.wedding.akad_time) {
    const akadDateTime = new Date(`${props.wedding.akad_date}T${props.wedding.akad_time}`)
    if (!isNaN(akadDateTime.getTime()) && akadDateTime > now) {
      return { date: props.wedding.akad_date, time: props.wedding.akad_time, label: 'Akad Nikah', timestamp: akadDateTime.getTime() }
    }
  }
  if (props.wedding.resepsi_date && props.wedding.resepsi_time) {
    const resepsiDateTime = new Date(`${props.wedding.resepsi_date}T${props.wedding.resepsi_time}`)
    if (!isNaN(resepsiDateTime.getTime()) && resepsiDateTime > now) {
      return { date: props.wedding.resepsi_date, time: props.wedding.resepsi_time, label: 'Resepsi', timestamp: resepsiDateTime.getTime() }
    }
  }
  return null
})
...
const formatTime = (time) => time ? time.substring(0, 5) + ' WIB' : ''
...
const openEnvelope = async () => {
  ...
  await nextTick()
  AOS.refresh()
  if (bgMusic.value) { try { await bgMusic.value.play(); isPlaying.value = true } catch (e) {} }
}
...
const copyRekening = async () => {
  try {
    await navigator.clipboard.writeText(props.wedding.rekening)
    isShaking.value = true
    setTimeout(() => isShaking.value = false, 500)
    showSuccess('Rekening disalin!')
  } catch { showError(null, 'Gagal menyalin') }
}

// API Calls
const loadMessages = async () => {
  const { data } = await supabase.from('messages').select('*').eq('wedding_id', props.wedding.id).order('created_at', { ascending: false }).limit(30)
  messages.value = data || []
}

const loadGifts = async () => {
  const { data } = await supabase.from('gifts').select(`*, dibeli_oleh (nama_tamu)`).eq('wedding_id', props.wedding.id).order('created_at')
  gifts.value = data.map(g => ({
    ...g, name: g.nama_barang, price: g.harga_estimasi, link: g.link_produk, image: g.gambar_url,
    buyer_name: g.dibeli_oleh?.nama_tamu, resiInput: '', claiming: false
  }))
}

const sendMessage = async () => {
  if (!newMessage.nama || !newMessage.pesan) return showError(null, 'Isi nama dan pesan')
  ...
  try {
    ...
    await loadMessages()
    showSuccess('Doa terkirim!')
  } catch { showError(null, 'Gagal mengirim') }
  finally { sendingMessage.value = false }
}

const submitRsvp = async () => {
  if (!props.guestSlug) return showError(null, 'Link tidak valid')
  submitting.value = true
  try {
    await supabase.from('guests').update({ status_hadir: rsvpStatus.value, jumlah_orang: rsvpStatus.value ? jumlahOrang.value : 0 }).eq('unique_slug', props.guestSlug)
    showSuccess('Terima kasih!')
    if (rsvpStatus.value) { showHearts.value = true; setTimeout(() => showHearts.value = false, 2500) }
  } catch { showError(null, 'Gagal') }
  finally { submitting.value = false }
}

const claimGift = async (gift) => {
  if (!gift.resiInput) return showError(null, 'Masukkan resi')
  gift.claiming = true
  try {
    ...
    gift.status = true; gift.buyer_name = props.guestName; gift.resi = gift.resiInput
    showSuccess('Kado diklaim!')
  } catch { showError(null, 'Gagal') }
  finally { gift.claiming = false }
}
...
onMounted(async () => {
  ...
  await loadGifts()
  updateMetaTags({
    title: `${props.wedding.nama_pria} & ${props.wedding.nama_wanita} - Wedding`,
    description: `Undangan pernikahan ${props.wedding.nama_pria} & ${props.wedding.nama_wanita}`,
    image: props.theme.gallery?.[0] || '/og-image.jpg'
  })
...
<style scoped>
/* Semua style elegan dari WeddingView lama */
...
.and-symbol { font-size: 36px; margin: 10px 0; opacity: 0.5; color: #9b87f5; }
...
@media (max-width: 480px) {
  ...
  .guest-name-display { font-size: 24px; }
